...
class ExerciseProcessor:
    def __init__(self, detection_strategy, angle_calculation_strategy, exercise_json_path, level=0):
        ...
        self.current_state = self.state_machine['states'][0]['id']
    ...
    def calculate_angle(self, angle_data, keypoint_coords):
        if "vertical" in angle_data and angle_data["vertical"]:
            ...
            # Исправление:  используем  dominant_side  для  определения  стороны  бедра
    ...
    def check_inactivity(self):
        if time.perf_counter() - self.state_tracker['start_inactive_time'] > self.inactivity_time:
    ...
    def _show_feedback(self, frame, feedback_indices, keypoint_coords):
        for idx in feedback_indices:
            message = self.feedback_messages[idx]
            self.cv_elem.draw_text(
                frame,
                message["message"],
                pos=(int(frame.shape[1] * 0.06), int(message["color"][0])),
            ...
            if "additional_drawing" in message:
                for drawing in message["additional_drawing"]:
                    if drawing["type"] == "line":
                        #  Используем  только  определенные  в  JSON  точки
    ...
    def _update_state_sequence(self, state):
        if state in self.state_machine['success_sequence']:
            if state not in self.state_tracker['state_seq'] or (self.state_tracker['state_seq'].count(state) < self.state_machine['success_sequence'].count(state)):
                self.state_tracker['state_seq'].append(state)
                if len(self.state_tracker['state_seq']) > len(self.state_machine['success_sequence']):
    ...
    def process(self, frame: np.array, curls=None, show_fps=False, plot=False):
        ...
        if len(keypoints):
            ...
            # Исправление:  объявляем  display_inactivity  перед  блоком  if
            ...
            if self.state_tracker['INCORRECT_POSTURE']:
                ...
                current_state = self.get_current_state(angles)
                self.state_tracker['curr_state'] = current_state
                self._update_state_sequence(current_state)
                print(f"\rcurr_state: {self.state_tracker['curr_state']}\tstate_seq: {self.state_tracker['state_seq']}", end='')

                # Count reps
                if current_state == self.state_machine["success_sequence"][0] and len(self.state_tracker['state_seq']) == len(self.state_machine["success_sequence"]):
                ...
                current_state_data = next((state for state in self.state_machine["states"] if state["id"] == current_state), None)

                feedback_indices = current_state_data["feedback"]
